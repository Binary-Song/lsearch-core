...
///
/// - `task_generator`: A function/closure that takes an `Yielder` and returns an async block (or anything that implements the `Future` trait).
///
///    *Yield* means producing intermediate results. You *yield* by calling `Yielder::yield_with` within the async block. The progress/intermediate results has type `TYield`.
///
///    The async block returns a final result of type `TFinal`.
///
/// - return value: A tuple of:
///   - A stream of `TaskUpdate<TFinal, TYield>` that yields progress updates and finally the result or error.
///   - An `AbortHandle` that can be used to abort the task.
fn spawn_task<TFinal, TYield, TFuture>(
    task_generator: impl FnOnce(Yielder<TYield>) -> TFuture,
        ...
        loop { // keep recving
            ...
            match select(join_fut, recv_fut).await {
                ...
                Either::Left((join_res, _)) => {
                    yield TaskUpdate::Final(join_res.unwrap()) ;
                    break;
                }
                // the task yields, continue recving
                Either::Right((recv_res, join_fut2)) => {
                    yield (TaskUpdate::Yield(recv_res.unwrap()));
                    join_fut = join_fut2;
                }
            }
        };
